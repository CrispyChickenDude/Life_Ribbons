...
import { Plus, Edit2, Trash2, X, Check, Calendar, Download, Upload, Image, FileJson, HelpCircle } from 'lucide-react';
...
const INITIAL_MILESTONES = [
  { id: 1, name: 'MBA', category: 'Education', startMonth: 9, startYear: 2022, endMonth: 5, endYear: 2026, ongoing: false, hypothetical: false, subEvents: [] },
  { id: 2, name: 'Flight Training', category: 'Hobby', startMonth: 3, startYear: 2019, endMonth: 8, endYear: 2023, ongoing: false, hypothetical: false, subEvents: [] },
  { id: 3, name: 'First Job', category: 'Career', startMonth: 6, startYear: 2015, endMonth: 12, endYear: 2019, ongoing: false, hypothetical: false, subEvents: [] },
...
export default function LifeNoodles() {
  ...
  const [importErrors, setImportErrors] = useState(null); // {imported: number, errors: [{name, reason}]}
  const [visibleCategories, setVisibleCategories] = useState(() => Object.keys(categories));
  const [editingId, setEditingId] = useState(null);
  const [hoveredId, setHoveredId] = useState(null);
  const [draggingEdge, setDraggingEdge] = useState(null); // { milestoneId, edge: 'start' | 'end' }
  const [dragPosition, setDragPosition] = useState(null); // { y, month, year }
  const [renamingId, setRenamingId] = useState(null); // ID of milestone being renamed
  ...
  useEffect(() => {
    ...
    // Add new categories to visible list
  ...
  const handleSubmit = (e) => {
    ...
    try {
      ...
      alert('There was an error saving the milestone. Please try again.');
    }
  };
  ...
  const updateCategory = () => {
    ...
    // Update category with new name and/or color
  ...
  // Helper function to generate filename with format: Ribbons-YYDDMM-HHMM
  const getExportFilename = (extension) => {
    ...
    return `Ribbons-${yy}${dd}${mm}-${hh}${min}.${extension}`;
  };

  // Export/Import Functions
  const exportData = () => {
    try {
      // Export as JSON with all data including categories
      const exportObj = {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (file.name.endsWith('.csv')) {
          ...
          if (categoryLines.length > 0) {
            ...
            categoryLines.forEach(line => {
              ...
              if (matches && matches.length >= 2) {
                ...
                if (catName && catColor) {
                  newCategories[catName] = catColor;
                }
              }
            });
          } else {
          ...
          if (Object.keys(newCategories).length > Object.keys(categories).length || categoryLines.length > 0) {
            setCategories(newCategories);
            // Make all categories visible by default
            setVisibleCategories(Object.keys(newCategories));
          }
          
          setMilestones(importedMilestones);
          
          alert(`CSV data imported successfully! ${importedMilestones.length} milestone(s) imported.`);
        } else {
          // Parse JSON
          ...
          // Ensure all milestones have required fields with defaults
          const milestonesWithDefaults = data.milestones.map(m => {
            // Auto-swap dates if needed for non-ongoing milestones
            let finalStartMonth = m.startMonth;
            let finalStartYear = m.startYear;
            let finalEndMonth = m.endMonth;
            let finalEndYear = m.endYear;
          ...
          setMilestones(milestonesWithDefaults);
          ...
          if (data.categories && typeof data.categories === 'object') {
            ...
            // Make all categories visible by default
            setVisibleCategories(Object.keys(data.categories));
          }
          
          
          alert(`JSON data imported successfully! ${milestonesWithDefaults.length} milestone(s) imported.`);
        }
      } catch (error) {
  ...
  const exportAsPNG = () => {
    try {
      ...
      img.onerror = (e) => {
        ...
        alert('Error loading SVG for PNG export. Please try again.');
        URL.revokeObjectURL(url);
      };
    ...
    <div style={{
      ...
      <main style={{
        ...
        <div className="responsive-container" style={{
          ...
          {/* SVG Timeline */}
          <div className="responsive-timeline" style={{ 
            position: 'relative',
            background: 'white',
            borderRadius: '32px',
            padding: '3rem 2rem',
            ...
            <svg 
              ref={svgRef}
              width={Math.max(800, totalLanes * 80 + 400)}
              height={timelineHeight + 40}
              viewBox={`${Math.min(0, 400 - (totalLanes * 80 / 2) - 150)} -40 ${Math.max(800, totalLanes * 80 + 400)} ${timelineHeight + 80}`}
              style={{ 
                overflow: 'visible', 
            ...
            >
              <defs>
                ...
                {Object.entries(categories).map(([cat, color]) => (
                  <linearGradient key={cat} id={`gradient-${cat}`} x1="0%" y1="0%" x2="0%" y2="100%">
              ...
              {Array.from({ length: maxYear - minYear + 1 }, (_, i) => minYear + i).map(year => (
                <g key={year}>
                  <line
                    x1={Math.min(50, 400 - (totalLanes * 80 / 2) - 100)}
                    y1={yearToY(year)}
                  ...
                  />
                  <text
                    x={Math.min(30, 400 - (totalLanes * 80 / 2) - 120)}
                    y={yearToY(year) + 5}
                    fill="#9CA3AF"
                    fontSize="14"
                    fontFamily="'Syne', sans-serif"
              ...
              {filteredMilestones.map((milestone, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <path
                      d={path}
                      fill={(milestone.hypothetical || false) ? 'none' : `url(#gradient-${milestone.category})`}
